...
def get_pie_chart(site_dropdown):
    ...
    if site_dropdown == 'ALL':
        ...
        names='Launch Site', 
        title='Success Launched for All')
        return fig
    else:
        # return the outcomes piechart for a selected site
        filtered_df = spacex_df[spacex_df["Launch Site"] == site_dropdown]
        pie = filtered_df.groupby(['Launch Site','class']).size().reset_index(name='class count')
        fig = px.pie(pie, values='class count', 
        names='class', 
        title=f"Success launches for {site_dropdown}")
        return fig
